...
import { NpmData, NpmHomepageResult } from '../../types';
import { createErrorResult, createSuccessResult } from '../util';

export async function npmGetHomepage(
  packageName: string
  ...
  return withCache(cacheKey, async () => {
    try {
      ...
      const homepageResult: NpmHomepageResult = {
        packageName: npmData.name,
        homepage: npmData.homepage,
      };

      return createSuccessResult(homepageResult);
    } catch (error) {
      return createErrorResult('Failed to get npm homepage information', error);
    }
  });
}
